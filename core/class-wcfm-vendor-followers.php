...
class WCFM_Vendor_Followers {
	...
	public function __construct() {
		...
		register_activation_hook( $WCFM->file, array( &$this,'wcfm_followers_my_account_flush_rewrite_rules' ) );
		register_deactivation_hook( $WCFM->file, array( &$this, 'wcfm_followers_my_account_flush_rewrite_rules' ) );
		...
		if( apply_filters( 'wcfm_is_allow_followers_new_product_notification', true ) ) {
			add_action( 'wcfm_after_new_product_by_vendor', array( &$this, 'wcfm_vendors_followers_notify_on_new_product' ), 10, 2 );
		}
		
		// Followers Settings
		//add_action( 'end_wcfm_settings', array( &$this, 'wcfm_followers_settings' ), 13 );
		//add_action( 'wcfm_settings_update', array( &$this, 'wcfm_followers_settings_update' ), 13 );
		
		// List Vendor Followers
    add_action( 'wp_ajax_wcfm_vendors_followers_list', array( &$this, 'wcfm_vendors_followers_list' ) );
		
		// Update Vendor Followers
    add_action( 'wp_ajax_wcfm_vendors_followers_update', array( &$this, 'wcfm_vendors_followers_update' ) );
    
    // Delete Followers
    add_action( 'wp_ajax_delete_wcfm_followers', array( &$this, 'wcfm_vendors_followers_delete' ) );
    
    // Delete Followings
    add_action( 'wp_ajax_delete_wcfm_followings', array( &$this, 'wcfm_vendors_followings_delete' ) );
  ...
  public function load_scripts( $end_point ) {
	  ...
	  switch( $end_point ) {
      	...
      	wp_enqueue_script( 'wcfm_followers_js', $WCFM->library->js_lib_url . 'followers/wcfm-script-followers.js', array('jquery'), $WCFM->version, true );
      	...
      	if( wcfm_is_vendor() ) {
	    	...
	    	wp_localize_script( 'wcfm_followers_js', 'wcfm_followers_screen_manage', $wcfm_screen_manager_data );
      break;
      
      case 'wcfm-followings':
      	wp_enqueue_script( 'wcfm_followings_js', $WCFM->library->js_lib_url . 'followers/wcfm-script-followings.js', array('jquery'), $WCFM->version, true );
      	// Localized Script
        //$wcfm_messages = get_wcfm_followers_manage_messages();
			  //wp_localize_script( 'wcfm_followers_manage_js', 'wcfm_followers_manage_messages', $wcfm_messages );
      break;
	  }
	}
   ...
   */
	public function load_styles( $end_point ) {
	  ...
	  switch( $end_point ) {
	  	case 'wcfm-followers':
		    wp_enqueue_style( 'wcfm_followers_css',  $WCFM->library->css_lib_url . 'followers/wcfm-style-followers.css', array(), $WCFM->version );
		  break;
		  
		  case 'wcfm-followings':
		  	wp_enqueue_style( 'wcfm_followings_css',  $WCFM->library->css_lib_url . 'followers/wcfm-style-followings.css', array(), $WCFM->version );
		  break;
	  }
	}
  ...
  public function load_views( $end_point ) {
	  ...
	  switch( $end_point ) {
	  	case 'wcfm-followers':
        $WCFM->template->get_template( 'followers/wcfm-view-followers.php' );
      break;
      
      case 'wcfm-followings':
        $WCFM->template->get_template( 'followers/wcfm-view-followings.php' );
      break;
	  }
	}
  ...
  public function ajax_controller() {
  	...
  	$controllers_path = $WCFM->plugin_path . 'controllers/followers/';
  	...
  	if( isset( $_POST['controller'] ) ) {
  		...
  		switch( $controller ) {
  			case 'wcfm-followers':
					include_once( $controllers_path . 'wcfm-controller-followers.php' );
					new WCFM_Followers_Controller();
				break;
				
				case 'wcfm-followings':
					include_once( $controllers_path . 'wcfm-controller-followings.php' );
					new WCFM_Followings_Controller();
				break;
  		}
  	}
  }
  ...
  function wcfm_followers_my_account_endpoints() {
	...
	function wcfm_followings_my_account_endpoint_content() {
		global $WCFM, $WCFM, $wpdb;
		include_once( $WCFM->library->views_path . 'followers/wcfm-view-my-account-followings.php' );
	}
	
	function wcfm_vendors_followers_notify_on_new_product( $product_id, $vendor_id ) {
		global $WCFM, $WCFM, $wpdb;
	...
	function wcfm_followers_settings( $wcfm_options ) {
		global $WCFM, $WCFM;
		
		$followers_icon = isset( $this->wcfm_followers_options['followers_icon'] ) ? $this->wcfm_followers_options['followers_icon'] : '';
		if( !$followers_icon ) $followers_icon = $WCFM->plugin_url . 'assets/images/verification_badge.png';
		...
		<div class="wcfm-container">
			<div id="wcfm_settings_form_followers_expander" class="wcfm-content">
				<?php
					$WCFM->wcfm_fields->wcfm_generate_form_field( apply_filters( 'wcfm_settings_fields_followers_general', array(
	...
	function wcfm_followers_settings_update( $wcfm_settings_form ) {
		global $WCFM, $WCFM, $_POST;
	...
	function wcfm_vendors_followers_list() {
		global $WCFM, $WCFM, $wpdb, $_POST;
	...
	function wcfm_vendors_followers_update() {
		global $WCFM, $WCFM, $wpdb, $_POST;
	...
	function wcfm_vendors_followers_delete() {
		global $WCFM, $WCFM, $wpdb, $_POST;
	...
	function wcfm_vendors_followings_delete() {
		global $WCFM, $WCFM, $wpdb, $_POST;
	...
	function wcfm_followers_scripts() {
 		global $WCFM, $WCFM, $wp, $WCFM_Query;
 		...
 		if( is_account_page() ) {
 			if( is_user_logged_in() ) {
				if( isset( $wp->query_vars[$this->wcfm_myaccount_followings_endpoint] ) ) {
					...
					wp_enqueue_script( 'wcfm_my_account_followings_js', $WCFM->library->js_lib_url . 'followers/wcfm-script-my-account-followings.js', array('jquery'), $WCFM->version, true );
					$wcfm_dashboard_messages = get_wcfm_dashboard_messages();
					wp_localize_script( 'wcfm_my_account_followings_js', 'wcfm_dashboard_messages', $wcfm_dashboard_messages );
				}
			}
 		}
 	}
 	...
 	function wcfm_followers_styles() {
 		...
 		if( is_account_page() ) {
 			if( is_user_logged_in() ) {
 				//if( isset( $wp->query_vars[$this->wcfm_myaccount_followings_endpoint] ) ) {
 					wp_enqueue_style( 'wcfm_my_account_support_manage_css',  $WCFM->library->css_lib_url . 'followers/wcfm-style-my-account-followings.css', array(), $WCFM->version );
 				//}
 			}
 		}
 	}
				...
				<script>
				  jQuery(document).ready(function($) {
				    $('#wcfm_follow_now').click(function(event) {
				      ...
				      var data = {
								action    : 'wcfm_vendors_followers_update',
								user_id   : $user_id,
								vendor_id : $vendor_id,
								count     : $count
							}	
				...
				<script>
					jQuery(document).ready(function($) {
						$('.wcfm_followers_label').click(function() {
							$vendor_id = $(this).data('vendor_id');
							var data = {
								action    : 'wcfm_vendors_followers_list',
								vendor_id : $vendor_id
							}	
							$.post(wcfm_params.ajax_url, data, function(response) {
								if(response) {
