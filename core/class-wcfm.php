		...
		if( apply_filters( 'wcfm_is_pref_vendor_followers', true ) ) {
			if ( !is_admin() || defined('DOING_AJAX') ) {
				if( $this->is_marketplace ) {
					...
					$this->wcfm_vendor_followers = new WCFM_Vendor_Followers();
				}
			}
		}
	...
	function restrict_wcfm_vendor_backend(){
	  global $WCFM, $_GET;
		if( is_user_logged_in() ) {
			...
			$is_export = false;
			$is_import = false;
			if( isset($_GET['page']) && ( wc_clean($_GET['page']) == 'product_exporter' ) ) { $is_export = true; }
			if( isset($_GET['page']) && ( wc_clean($_GET['page']) == 'product_importer' ) ) { $is_import = true; }
			if( isset($_GET['page']) && ( wc_clean($_GET['page']) == 'store-setup' ) ) { $is_setup = true; }
